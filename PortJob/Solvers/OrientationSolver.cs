            ...
            foreach (FLVER2.Mesh flverMesh in flver.Meshes) {
                for (int i = 0; i < flverMesh.Vertices.Count; i++) {
                    ...
                    Matrix4x4 m = Matrix4x4.Identity
                                  * Matrix4x4.CreateRotationY(0) // Set all these to 0 since I don't think we will need scene rotation for what we are doing.
                                  * Matrix4x4.CreateRotationZ(0)
                                  * Matrix4x4.CreateRotationX(0)
                    ;

                    flverMesh.Vertices[i].Position = Vector3.Transform(new Vector3(flverMesh.Vertices[i].Position.X, flverMesh.Vertices[i].Position.Y, flverMesh.Vertices[i].Position.Z), m);
